...
func New(ctx context.Context, name string, config map[string]string, metrics *observability.Metrics) (Backend, error) {
	op, ctx := observability.StartOperation(ctx, metrics, "blobstore.physical.new")
	var err error
	defer op.End(err)

	slog.InfoContext(ctx, "creating blobstore backend", "backend", name)
	...
	if !ok {
		err = storage.NewConfigError(name, "", fmt.Sprintf("unknown blobstore backend %q (available: %v)", name, ListBackends()))
		return nil, err
	}
	...
	slog.InfoContext(ctx, "blobstore backend created", "backend", name)
	return backend, nil
}
