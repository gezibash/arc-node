...
func newReadModel(ctx context.Context, sdk *dm.DM, msg dm.Message, layout *tui.Layout) readModel {
	s := spinner.New()
...
func (m readModel) renderSubHeader() string {
	short := reference.Hex(m.msg.Ref)[:8]
	ts := time.Unix(0, m.msg.Timestamp)
